...
@section('page_title', 'Expense by Category')
...
<div class="report-premium-container"
     x-data="{
        ...
        exportType: 'pdf',
        buildPdfUrl() {
            let base = '{{ route('reports.expense_category_report.pdf') }}';
            let params = new URLSearchParams(@js(request()->query()));
            return base + '?' + params.toString();
        },
        buildExcelUrl() {
            let base = '{{ route('reports.expense_category_report.excel') }}';
            let params = new URLSearchParams(@js(request()->query()));
            return base + '?' + params.toString();
        }
     }">
    ...
    <div class="report-premium-header no-print">
        <div>
            <h1 class="report-premium-title">Expense Category Report</h1>
            <p class="report-premium-subtitle">Analysis of expenses grouped by account category</p>
        </div>
    ...
    <div class="report-premium-stat-grid">
        ...
        <div class="bg-white p-5 rounded-[1rem] border border-gray-100 shadow-sm flex items-start justify-between hover:-translate-y-0.5 transition-transform duration-200">
            <div>
                <p class="text-[12px] text-gray-400 font-medium mb-1">Highest Expense Category</p>
                <h3 class="text-[18px] font-black text-primary">
                    {{ $reportData->first()->category_name ?? 'N/A' }}
                </h3>
                <p class="text-xs font-bold text-primary-dark mt-1.5 flex items-center gap-1">By Value</p>
            </div>
            <div class="w-11 h-11 bg-primary/10 rounded-[0.6rem] flex items-center justify-center text-primary flex-shrink-0">
        ...
        <div class="bg-white p-5 rounded-[1rem] border border-gray-100 shadow-sm flex items-start justify-between hover:-translate-y-0.5 transition-transform duration-200">
            <div>
                ...
                <h3 class="text-[18px] font-black text-primary">{{ number_format($reportData->sum('transaction_count')) }}</h3>
                <p class="text-xs font-bold text-primary-dark mt-1.5 flex items-center gap-1">Recorded Vouchers</p>
            </div>
            <div class="w-11 h-11 bg-primary/10 rounded-[0.6rem] flex items-center justify-center text-primary flex-shrink-0">
        ...
        <div class="bg-white p-5 rounded-[1rem] border border-gray-100 shadow-sm flex items-start justify-between hover:-translate-y-0.5 transition-transform duration-200">
            <div>
                <p class="text-[12px] text-gray-400 font-medium mb-1">Active Categories</p>
                <h3 class="text-[18px] font-black text-primary">{{ $reportData->count() }}</h3>
                <p class="text-xs font-bold text-primary-dark mt-1.5 flex items-center gap-1">Unique Accounts</p>
            </div>
            <div class="w-11 h-11 bg-primary/10 rounded-[0.6rem] flex items-center justify-center text-primary flex-shrink-0">
    ...
    <!-- Filter Bar -->
    <form action="{{ route('reports.expense_category_report') }}" method="GET" class="report-premium-filter-bar no-print">
    ...
    <div class="report-premium-card overflow-hidden mb-6">
        <div class="px-5 py-4 border-b border-brand-border bg-brand-bg/10 flex justify-between items-center">
            <div class="flex items-center gap-2">
                ...
                <h4 class="text-[11px] font-black text-text-secondary uppercase tracking-widest">Expense Details</h4>
            </div>
            ...
            <table class="report-premium-table">
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td>
                            <div class="font-bold text-primary-dark text-[13px]">{{ $row->category_name }}</div>
                        </td>
                        <td>
                            <span class="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-[10px] font-bold">{{ $row->category_code }}</span>
                        </td>
                        <td class="text-center font-bold text-primary-dark">{{ $row->transaction_count }}</td>
                    ...
                    <tr>
                        <td colspan="6" class="px-5 py-20 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-gray-400 font-medium text-sm">No expense records found for this period</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
                ...
                <tfoot class="border-t-2 border-primary/10">
                    <tr class="bg-gray-50/50 font-black text-primary-dark">
                        ...
                        <td class="px-5 py-4 text-center text-[14px]">{{ number_format($reportData->sum('transaction_count')) }}</td>
