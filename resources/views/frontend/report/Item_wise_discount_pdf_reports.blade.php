...
<head>
    <meta charset="UTF-8">
    <title>Item-Wise Discount Report — {{ $company->name ?? 'Company' }}</title>
    <style>
...
<body>
    ...
    <div class="header">
        ...
        <div>
            ...
            <div style="float: left;">
                ...
                <div class="report-title">Item-Wise Discount Report</div>
            </div>
        </div>
        <div class="clear"></div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="right">{{ number_format($row->total_sale_amount, 2) }}</td>
                <td class="right red">{{ number_format($row->total_discount_amount, 2) }}</td>
                <td class="right">
                    {{ $row->total_sale_amount > 0 ? number_format(($row->total_discount_amount / $row->total_sale_amount) * 100, 1) : 0 }}%
                </td>
            </tr>
            @endforeach
        </tbody>
        <tfoot>
            <tr>
                ...
                <td style="text-align: right;">{{ number_format($totals->sale_amount, 2) }}</td>
                <td style="text-align: right; color: #dc2626;">{{ number_format($totals->discount_amount, 2) }}</td>
    ...
    <div class="footer">
        {{ $company->name ?? 'Company' }} — Item-Wise Discount Analysis — Confidential
    </div>
