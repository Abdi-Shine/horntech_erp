...
<head>
    ...
    <title>Purchase Report — {{ $company->name ?? 'Company' }}</title>
    <style>
...
<body>
    ...
    <div class="header">
        ...
        <div class="report-meta">
            ...
            @endif
            <p><strong>Total Records:</strong> {{ number_format($totalBills) }}</p>
        </div>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                @if($cols->contains('invoice_no'))  <th>Bill #</th>        @endif
                @if($cols->contains('date'))         <th>Date</th>          @endif
                @if($cols->contains('party_name'))   <th>Supplier</th>      @endif
        ...
        <tbody>
            ...
            <tr>
                ...
                @if($cols->contains('invoice_no'))   <td class="bold">{{ $purchase->bill_number }}</td> @endif
                @if($cols->contains('date'))         <td>{{ \Carbon\Carbon::parse($purchase->bill_date)->format('d-M-Y') }}</td> @endif
                @if($cols->contains('party_name'))   <td class="bold">{{ $supplierName }}</td> @endif
                @if($cols->contains('phone'))        <td>{{ $phone }}</td> @endif
                @if($cols->contains('order_number')) <td>{{ $purchase->id }}</td> @endif
                @if($cols->contains('item_details')) <td class="center">{{ number_format($purchase->items->sum('quantity')) }}</td> @endif
                @if($cols->contains('description'))  <td>{{ $purchase->notes ?? '—' }}</td> @endif
                @if($cols->contains('payment_type')) <td class="center">{{ $purchase->payment_method ?? 'Cash' }}</td> @endif
                @if($cols->contains('total'))        <td class="right bold">{{ number_format($purchase->total_amount, 2) }}</td> @endif
                @if($cols->contains('received'))     <td class="right green">{{ number_format($purchase->paid_amount, 2) }}</td> @endif
                @if($cols->contains('balance'))      <td class="right red">{{ number_format($purchase->balance_amount, 2) }}</td> @endif
                @if($cols->contains('payment_status')) <td class="center"><span class="pill {{ $pillClass }}">{{ ucfirst($purchase->status) }}</span></td> @endif
            </tr>
            @empty
            <tr>
        ...
        <tfoot>
            <tr>
                ...
                @if($cols->contains('total'))         <td class="right">{{ number_format($totalPurchases, 2) }}</td>   @endif
    ...
    <div class="footer">
        ...
        <span>Generated by Purchase Report System</span>
    </div>
